...
const u_char* find_Sender_Mac(pcap* handle, pcap_pkthdr* header, ARP *Sen_ARP_Pac,const u_char* packet)
{
    while (true) { // if it receives packet, break;
         ...
         printf("\n");
         printf("SHW_Adr:");
          for(int i=0; i<6; i++)printf("%02x",Sen_ARP_Pac->ARP_Header.SHW_Adr[i]);
          printf("\n");
          printf("SenderIP:%02x",sender_ip);
          printf("AttackerIP:%02x",attacker_ip);
  ...
  \
                &&(Sen_ARP_Pac->ARP_Header.SPT_Adr == ntohl(attacker_ip)))    //&& (strcmp((const char *)Dst_mac,(const char *)Sen_ARP_Pac->ARP_Header.SHW_Adr)==0))
          {
            return packet;
          }
         else
...
const u_char * find_Target_Mac(pcap* handle, pcap_pkthdr* header, ARP *Tar_ARP_Pac,const u_char* packet)
{
    ...
    while (true) { // if it receives packet, break;
        ...
        if((ARP_Number==eth_type)&& (OC_Rep== OpCode)&& (Tar_ARP_Pac->ARP_Header.DPT_Adr ==ntohl(target_ip))
           &&(Tar_ARP_Pac->ARP_Header.SPT_Adr == ntohl(attacker_ip)))
